...
"""
from ..clkdesc import ClkDescription, ClockType, Clock, Div, Mux
from .abstractfilter import AbstractFilter, Property, State

class QueryFilter(AbstractFilter):
    def __init__(self, graph: ClkDescription, query: ClockType,
        ...
        def find_successors(graph: ClkDescription, path: list[ClockType]) -> set[ClockType]:
            """I am aware that this is not efficient, but our graphs are small"""
            all_outputs = {path[-1]}
